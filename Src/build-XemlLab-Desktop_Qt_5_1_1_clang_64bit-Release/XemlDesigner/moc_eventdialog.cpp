/****************************************************************************
** Meta object code from reading C++ file 'eventdialog.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.1.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../XemlLab/XemlDesigner/DialogWindows/eventdialog.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'eventdialog.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.1.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_EventDialog_t {
    QByteArrayData data[9];
    char stringdata[77];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_EventDialog_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_EventDialog_t qt_meta_stringdata_EventDialog = {
    {
QT_MOC_LITERAL(0, 0, 11),
QT_MOC_LITERAL(1, 12, 9),
QT_MOC_LITERAL(2, 22, 0),
QT_MOC_LITERAL(3, 23, 6),
QT_MOC_LITERAL(4, 30, 1),
QT_MOC_LITERAL(5, 32, 12),
QT_MOC_LITERAL(6, 45, 9),
QT_MOC_LITERAL(7, 55, 15),
QT_MOC_LITERAL(8, 71, 4)
    },
    "EventDialog\0event_set\0\0Event*\0e\0"
    "event_edited\0OkClicked\0enabledOkButton\0"
    "text\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_EventDialog[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   34,    2, 0x05,
       5,    0,   37,    2, 0x05,

 // slots: name, argc, parameters, tag, flags
       6,    0,   38,    2, 0x08,
       7,    1,   39,    2, 0x08,

 // signals: parameters
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString,    8,

       0        // eod
};

void EventDialog::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        EventDialog *_t = static_cast<EventDialog *>(_o);
        switch (_id) {
        case 0: _t->event_set((*reinterpret_cast< Event*(*)>(_a[1]))); break;
        case 1: _t->event_edited(); break;
        case 2: _t->OkClicked(); break;
        case 3: _t->enabledOkButton((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (EventDialog::*_t)(Event * );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&EventDialog::event_set)) {
                *result = 0;
            }
        }
        {
            typedef void (EventDialog::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&EventDialog::event_edited)) {
                *result = 1;
            }
        }
    }
}

const QMetaObject EventDialog::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_EventDialog.data,
      qt_meta_data_EventDialog,  qt_static_metacall, 0, 0}
};


const QMetaObject *EventDialog::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *EventDialog::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_EventDialog.stringdata))
        return static_cast<void*>(const_cast< EventDialog*>(this));
    return QDialog::qt_metacast(_clname);
}

int EventDialog::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void EventDialog::event_set(Event * _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void EventDialog::event_edited()
{
    QMetaObject::activate(this, &staticMetaObject, 1, 0);
}
QT_END_MOC_NAMESPACE
