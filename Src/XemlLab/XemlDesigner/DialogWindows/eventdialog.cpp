...
EventDialog::EventDialog(bool _mode,GraphicEventItem  * _event,ItfDocument * _xemldoc,StoryBase * _story,QWidget * parent)
	: QDialog(parent)
{
	this->Mode=_mode;//Edit a existing one or create a new one
	this->event=_event->get_event();
	this->graphicEvent=_event;
	this->current_doc=_xemldoc;
	this->current_story=_story;
	this->label = new QLabel(tr("Label :"));
	this->lineEdit = new QLineEdit;
