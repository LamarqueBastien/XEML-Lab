...
: QWizardPage(parent)
{
	setTitle(tr("Add individuals identifier from <i>CSV files</i>;"));
	...
	//registerField("headerViewSelection2", this, "selectionModel");
	...
	mainlayout->addWidget(headerviews);
	setLayout(mainlayout);
	setWindowTitle("import Individual id from files ");
}
void IndividualImportFilePage::process_line(const QString &)
{

}
void IndividualImportFilePage::open_file(){
...
void IndividualImportFilePage::load_file(){
	...
	if (!fileName.isEmpty()) {
		...
		if(!IndividualWizard::get_CSV_data()->empty()){
			IndividualWizard::get_CSV_data()->clear();
		}
		tmpvec=IndividualWizard::get_CSV_data();
...
bool IndividualImportFilePage::checkForDelimiters(int _count_line,QString &_word, QString _dataDelimiter,QString _textDelimiter,QChar character){
